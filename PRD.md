...
- **换 IP 冷却处理（v1.2）**：生产观测到提交受理后 `main_ip` 在整个观察窗口内始终未变
  （曾持续数小时），判定为 Hostwinds 侧冷却/限频；此时**立即停止本次运行**，不再连刷，
  交由下次 30 分钟 timer 稀疏重试。因此 `--max-cycles` 默认由 5 降为 2。
- **瞬时超时容错（v1.1）**：`get_instance` 单次超时不再中断整个换 IP 循环——
  新增 `get_instance_retry`，重试 `submit-retries` 次仍失败才跳过本轮（而非直接退出）。
...
### 6.5 节点服务栈可能是 v2ray 而非 Xray（v1.4）

`deploy.sh`/`remote-setup.sh.tmpl` 按 **Xray** 编写，但换到新实例后发现该节点是
**手动部署的 v2ray**（配置路径、服务名均不同于 Xray）。不能假设新节点一定是 Xray，
也不能直接对已有部署重跑 `remote-setup.sh.tmpl`（会冲突）。

确认某节点实际用哪套的方法：`ss -tlnp | grep <vmess端口>` 找到监听进程名及其
`-config` 参数指向的真实配置文件。

**IPv6 出口问题**：部分机房的 IPv6 段容易被目标网站判定为异常流量（观测样本：
访问 Google/Gemini 报 "unusual traffic"，出口地址为 IPv6）。已将 v2ray/Xray 出站
`freedom` 协议的 `domainStrategy` 由 `UseIP` 改为 `UseIPv4`，强制代理出站只走
IPv4，问题未再复现。此为**节点级配置**，每次迁移到新实例都需要重新检查并应用。
...
| `--retry-cooldown` | 30s | |
| `--submit-retries` | 3 | |
| `--max-cycles` | 2（v1.2 由 5 下调） | 免费但避免在冷却期连刷；配合冷却即停策略，靠 timer 稀疏重试 |
...
| L7 | **部署脚本假设与实际节点可能不符**（v1.4 新增） | `deploy.sh`/`remote-setup.sh.tmpl` 假设 Xray，但节点可能是手动部署的 v2ray（路径、服务名均不同，见 §6.5）；换新实例时必须重新确认 |
| L8 | **IPv6 出口需逐节点手动配置**（v1.4 新增） | `domainStrategy: UseIPv4` 是节点级配置，换新实例后需重新检查并设置，自动化流程未覆盖此项 |
...
- **节点迁移**：旧实例换 IP 卡在同一 IP 长时间不再变化（远超正常冷却时长），判定为
  该实例异常，改为创建全新实例替代，API key 一并轮换。
- **发现**：新实例是手动部署的 **v2ray**（非 `deploy.sh` 假设的 Xray），配置路径、
  服务名均不同，详见 §6.5。订阅文件曾使用错误的端口/UUID（创建实例时的默认占位值），
  已按实际部署更正，并通过服务器侧 TCP 探测交叉验证（本地终端的网络探测因被本机
  代理/VPN 接管出现全端口假阳性，不可作为验证依据，需从订阅服务器或节点自身发起探测）。
- **修复**：出站 `domainStrategy` 由 `UseIP` 改为 `UseIPv4`，解决目标网站将部分机房
  IPv6 出口判定为异常流量的问题。
- 新增 §6.5、L7、L8，记录部署脚本假设与实际节点可能不符、IPv6 出口需逐节点配置。
